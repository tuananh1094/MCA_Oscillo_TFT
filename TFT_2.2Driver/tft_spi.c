...
void LCD_GPIO_Init(void)
{
	...
	//OUTPUT
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2|GPIO_Pin_3|GPIO_Pin_4|GPIO_Pin_6|GPIO_Pin_7|GPIO_Pin_5;	
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT ;		// Output mode
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	// High speed IO
...
void SPI1_Init(void)	
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4| GPIO_Pin_2| GPIO_Pin_3| GPIO_Pin_6;
	...
	SPI_InitStructure.SPI_Direction = SPI_Direction_Tx;//SPI_Direction_2Lines_FullDuplex;
...
void Lcd_ClearRigion(u8 x, u8 y, u8 pixelx, u8 pixely, u16 color)
{
    ...
    for(i=x+1;i<pixelx+x;i++)
    for(m=x+1;m<pixely+y;m++)
    {	
	  	Lcd_WriteData_16Bit(color);
    } 
}
